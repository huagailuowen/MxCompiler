...
总体上本项目分为 3 个阶段分别评测：语义检查（Semantic Check），代码生成（ASMNaive / Code Generation），代码优化（Optimization）。
...
3. **调用第三方库**
  - 界定：你的代码不应该调用第三方库用于任何阶段，例如生成 LLVM IR 后直接调用 Clang 等工具完成 ASMNaive 等工作是违规的
